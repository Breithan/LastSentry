...
public class SC_BossBS3 : MonoBehaviour {
    ...
    public void ExplodeSkill()
    {
		if (fltBullet < 5)
		{
			GameObject activeM1 = Instantiate(gobMissilRed, new Vector2(this.transform.position.x - 1.15f, this.transform.position.y - 1.5f), this.transform.rotation);
            activeM1.GetComponent<Rigidbody2D>().velocity = new Vector2(0, -15);

            GameObject activeM2 = Instantiate(gobMissilRed, new Vector2(this.transform.position.x + 1.15f, this.transform.position.y - 1.5f), this.transform.rotation);
